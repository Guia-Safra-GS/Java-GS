...
Global Solution 2026/1 — FIAP — Análise e Desenvolvimento de Sistemas </br>
...
|---|---|---|---|
| `User` | TB_CAD_USER | Java | Produtores e operadores do sistema |
| `ClimateForecast` | TB_MON_CLIMATE_FORECAST | Java | Previsão do tempo por dia, com risco de geada |
| `Reading` | TB_MON_READING | Java | Cada medição de umidade/temperatura de um sensor |
| `WateringEvent` | TB_MON_WATERING_EVENT | Java | Cada rega ocorrida (manual ou automática) |
| `Alert` | TB_MON_ALERT | Java | Avisos de umidade crítica ou risco de geada |
| `Slot` | TB_CAD_SLOT | .NET | Canteiro físico (lido pelo Java, nunca gravado) |
...
foram extraídos para uma classe-base abstrata `Event`, da qual `WateringEvent` e `Alert`
herdam.
...
Observação sobre o banco: a aplicação roda com `ddl-auto=validate`, ou seja, ela não cria nem
altera tabelas — apenas confere se o mapeamento das entidades bate com o schema existente. As
tabelas precisam ter sido criadas antes (o script de criação está no repositório da solução, em
`gs_agromonitor.sql`).
